    ...
    async createPool(year: number, membersInput: PoolMemberInput[]) {
        ...
        if (totalCb < 0) {
            throw new BusinessRuleError(
                'INVALID_POOL',
                `Pool total CB is negative (${totalCb.toFixed(2)}). Sum of all members' CBs must be ≥ 0.`
            );
        }
        ...
        for (const member of workingList) {
            ...
            if (member.cbBefore < 0 && member.cbAfter < member.cbBefore) {
                throw new BusinessRuleError(
                    'INVALID_POOL',
                    `Deficit member ${member.shipId} cannot exit worse (before: ${member.cbBefore}, after: ${member.cbAfter})`
                );
            }
            ...
            if (member.cbBefore > 0 && member.cbAfter < 0) {
                throw new BusinessRuleError(
                    'INVALID_POOL',
                    `Surplus member ${member.shipId} cannot exit negative (cbAfter: ${member.cbAfter})`
                );
            }
        }
